...
class PartDataset(data.Dataset):
    def __init__(self, root=dataset_path, which_dataset='', npoints=2500, classification=False,
        ...
        if which_dataset == 'PCN':
            self.catfile = os.path.join(self.root, 'dataset/PCN.json')
        else:
        ...
        for item in self.cat:
            ...
            if which_dataset == 'PCN':
                dir_point = os.path.join(self.root, 'test/complete', self.cat[item])
                dir_seg = os.path.join(self.root, 'test/complete', self.cat[item])
            else:
    ...
    def __getitem__(self, index):
        if index in self.cache:
            ...
            if self.which_dataset == 'PCN':
                seg = np.loadtxt(fn[2], skiprows=11).astype(np.int64) - 1
            else:
