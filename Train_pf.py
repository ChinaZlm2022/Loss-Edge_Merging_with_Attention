    ...
    for epoch in range(resume_epoch, opt.niter):
        ...
        for i, data in enumerate(dataloader, 0):
            ...
            for i in range(b):
                ...
                # 假设已经定义了 density_ton1 和 n_clusters 等变量
                while True:
            ...
            errD_real.backward()
            fake_center1, fake_center2, fake = point_netG(input_cropped, combined_tensor)
        ...
        if epoch % 10 == 0:
            ...
            torch.save({'epoch': epoch + 1,
                        'state_dict': point_netG.state_dict()},
                       '/root/autodl-zm/zm-test01/Trained_Model/point_netG' + str(
                           epoch) + '.pth')
            torch.save({'epoch': epoch + 1,
                        'state_dict': point_netD.state_dict()},
                       '/root/autodl-zm/zm-test01/Trained_Model/point_netD' + str(
                           epoch) + '.pth')
